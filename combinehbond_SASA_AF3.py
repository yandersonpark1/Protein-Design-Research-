...
class combinehbond_SASA_AF3: 
    ...
    def __init__(self, AF3_file, hbond_file, SASA_file): 
        ...
        self.AF3_file = pd.read_excel(AF3_file, header = 0, usecols=['diff', 'bb', 'seq #', 'seq name', 'plDDT', 'plDDT A', 'plDDT B', 'pTM', 'ipTM', 'evo pro', 'ctct score', '# ctct', 'PAE/ ctct'])
    
    def combine(self, version): 
        combined = pd.concat([self.AF3_file, self.hbond_df, self.SASA_file], axis=1)
        combined.to_excel(f'{version}_metrics.xlsx', index=False)
