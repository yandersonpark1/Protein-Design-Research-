...
class LMPNN_Af3proteinfilter(): 
    ...
    def __init__(self, input_file): 
        ...
        self.df = pd.read_excel(input_file, header=0, usecols=['diff', 'redesign', 'seq #', 'seq name', 'ligand', 'plDDT', 'plDDT A', 'plDDT B', 'pTM', 'ipTM', 'evo pro', 'ctct score', '# ctct', 'PAE/ ctct', '# ctct@residue', 'PAE/ ctct@residue','ctct score@residue', 'hbond count', 'hbond b1', 'hbond b2', 'hbond b3', 'B1 total SASA', 'b1 bb', 'b1 sc', 'B2 total SASA', 'b2 bb', 'b2 sc', 'B3 total SASA', 'b3 bb', 'b3 sc'])
        self.filtered_df = self.df.copy()
    
    def plDDT(self, min_plDDT = 85, column = "plDDT A"): 
        """
    ...
    def ipTM(self, min_ipTM = 0.7, column = "ipTM"): 
        """
    ...
    def save_filtered_data(self, version, scaffold):
        ...
        """
        output_file=f"redesign_{version}_{scaffold}_filtered.xlsx"
...
def main(): 
    ...
    scaffold = str(input(("Enter the scaffold name (e.g., ClpS): ")))
