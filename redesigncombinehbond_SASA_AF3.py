...
class combinehbond_SASA_AF3: 
    ...
    def __init__(self, AF3_file, hbond_file, SASA_file): 
        ...
        self.AF3_file = pd.read_excel(AF3_file, header = 0, usecols=['diff', 'redesign', 'seq #', 'seq name', 'ligand', 'plDDT', 'plDDT A', 'plDDT B', 'pTM', 'ipTM', 'evo pro', 'ctct score', '# ctct', 'PAE/ ctct', '# ctct@residue', 'PAE/ ctct@residue','ctct score@residue'])
    
    def combine(self, version, scaffold): 
        combined = pd.concat([self.AF3_file, self.hbond_df, self.SASA_file], axis=1)
        combined.to_excel(f'redesign_{version}_{scaffold}_metrics.xlsx', index=False)
